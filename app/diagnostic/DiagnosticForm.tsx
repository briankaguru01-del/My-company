...
export default function DiagnosticForm() {
  ...
  if (status === "success") {
    return (
      <div
        role="status"
        className="rounded-lg border border-signal-500/30 bg-signal-500/5 p-8 text-center sm:p-12"
      >
  ...
  return (
    ...
    >
            ...
            <div key={field.name} className={wrapperClass}>
              ...
              {field.type === "textarea" ? (
                <textarea
                  ...
                  className="mt-2 block w-full rounded-md border border-ink-200 bg-white px-4 py-3 text-sm text-ink-950 placeholder:text-ink-400 focus:border-ink-950 focus:outline-none"
                  {...register(field.name)}
                />
              ) : field.type === "select" ? (
                <select
                  ...
                  aria-invalid={error ? "true" : "false"}
                  aria-describedby={describedBy}
                  className="mt-2 block w-full rounded-md border border-ink-200 bg-white px-4 py-3 text-sm text-ink-950 focus:border-ink-950 focus:outline-none"
                  {...register(field.name)}
                >
      ...
      <button
        ...
        className="inline-flex w-full items-center justify-center rounded-lg bg-ink-950 px-7 py-4 text-sm font-medium tracking-wide text-white transition-colors duration-150 hover:bg-ink-800 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
      >
